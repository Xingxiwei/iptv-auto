...
SOURCE_URLS = [
    ...
    "https://raw.githubusercontent.com/imDazui/Tvlist-awesome-m3u-m3u8/refs/heads/master/m3u/%E5%8F%B0%E6%B9%BE%E9%A6%99%E6%B8%AF%E6%B5%B7%E5%A4%99.m3u",
...
KEYWORDS = ["ViuTV", "HOY", "RTHK", "Jade", "Pearl", "J2", "J5", "Now", "无线", "無線", "有线", "有線", "翡翠", "明珠", 
            "港台", "廣東", "珠江", "广州", "廣州", "大灣區","鳳凰", "凤凰","成人", "民視", "東森", "三立", "中視", "公視", 
            "TVBS", "緯來", "年代", "中天", "非凡", "澳視", "澳門", "TDM", "澳亞"]

# 4. 黑名單：名入面有呢啲字就一定唔要 (剔除購物、測試台等)
BLOCK_KEYWORDS = ["FOX", "Pluto", "Local", "NBC", "CBS", "ABC", "AXS", "Snowy", "Reuters", "Mirror", "ET Now", "The Now", 
                  "Right Now", "News Now", "Chopper", "Wow", "UHD", "8K", "Career", "Comics", "Movies", "CBTV","Pearl",
                  "AccuWeather","Jadeed","Curiosity","Electric", "Warfare","Knowledge","MagellanTV","70s","80s","90s","Rock",
                  "Winnipeg","Edmonton","RightNow","Times","True","Mindanow", "浙江", "杭州", "西湖", "深圳", "韶關", "CCTV", 
                  "CGTN", "華麗", "星河", "延时", "測試", "iHOY", "福建"]

# 5. 排序優先級：越排前面嘅字，喺 M3U 播放器入面就會排得越靠上
ORDER_KEYWORDS = ["廣東", "珠江", "廣州", "廣東衛視", "大灣區", "南方", "港台電視", "翡翠", "無線新聞", "明珠", "J2", "J5", 
                  "財經", "Viu", "HOY", "奇妙", "有線", "Now", "民視", "中視", "華視", "公視", "TVBS", "三立", "東森", "年代", 
                  "壹電視", "非凡", "中天", "緯來", "澳視", "澳門", "TDM", "澳亞"]

# 6. 靜態源：絕對穩定、唔使 check 嘅官方 Link，放喺列表最頭
STATIC_CHANNELS = [
...
def check_url(item):
    """
    【功能】檢查單個網址係咪仲行得通
    1. 先試 HEAD (淨係讀 Header，快好多)
    2. 如果 HEAD 唔得就試 GET (對付一啲唔俾人用 HEAD 掃描嘅伺服器)
    """
    url = item['url']
    headers = COMMON_HEADERS.copy()
    headers['Referer'] = url # 模擬來源地址，破解部分防盜鏈
    try:
        # allow_redirects=True 係為咗處理跳轉 Link
        response = requests.head(url, timeout=2, headers=headers, allow_redirects=True)
        if response.status_code == 200: return item
        
        # 某啲源阻擋 HEAD，要用 GET stream 模式（只讀開頭就斷開）
        response = requests.get(url, timeout=3, headers=headers, stream=True)
        if response.status_code == 200:
            response.close() # 檢查完即刻斷開，慳流量
            return item
    except: pass
    return None
...
def generate_m3u(channels):
    ...
    # 【多線程】20 條線程同時開工 Check Link，唔使一條一條等，速度快 20 倍
    ...
    print(f"\n✅ 檢測完成！共收錄 {len(valid_channels)} 個有效頻道。", flush=True)

    # 根據權重進行排序
    print("🔄 正在進行排序...", flush=True)
    final_list.sort(key=get_sort_key)

    # 【寫入檔案】生成標準 M3U 格式，帶埋 EPG 同 Logo 網址
